<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
      <div class="flex justify-between items-center mb-4">
        <div>
          ...
          <p class="text-gray-600">Track your study progress and patterns</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-semibold text-gray-900">Weekly Study Time</h2>
          <BarChart3 class="h-5 w-5 text-gray-500" />
        </div>
        
        <div class="space-y-4">
          <div v-for="(day, index) in weeklyData" :key="index" class="flex items-center space-x-4">
            <div class="w-12 text-sm font-medium text-gray-600">{{ day.day }}</div>
            <div class="flex-1 relative">
              <div class="w-full bg-gray-200 rounded-full h-6">
                ...
                >
                  <span class="text-white text-xs font-medium">{{ day.hours }}h</span>
                </div>
              </div>
            </div>
            <div class="text-sm text-gray-500 w-16">{{ day.sessions }} sessions</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-6">Subject Distribution</h2>
        
        <div class="space-y-4">
          <div v-for="(subject, index) in subjectData" :key="index" class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
              <div class="w-24 bg-gray-200 rounded-full h-2">
                <div
                  :class="`h-2 rounded-full transition-all duration-500 ${subject.color}`"
                  :style="{ width: `${subject.percentage}%` }"
                ></div>
              </div>
              <span class="text-sm text-gray-600 w-12">{{ subject.hours }}h</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <h2 class="text-xl font-semibold text-gray-900 mb-6">Hourly Activity Heatmap</h2>
      
      <div class="grid grid-cols-6 md:grid-cols-9 lg:grid-cols-18 gap-2 mb-4">
        <div v-for="(hour, index) in hourlyActivity" :key="index" class="text-center">
          <div
            :class="`w-8 h-8 rounded-md mb-1 transition-all duration-200 hover:scale-110 ${getHeatmapIntensity(hour.value)}`"
...
<script>
import { ref } from 'vue'
import { BarChart3, TrendingUp, Clock, Target, Calendar, Filter } from 'lucide-vue-next'
